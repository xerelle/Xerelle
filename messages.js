...
// Sends a message. Enforces the core paywall rule:
//   - A model can always message a subscriber she has an active
//     subscription OR a still-unused trial message with.
//   - A subscriber can send exactly ONE free trial message per model,
//     ever (no reset, whether or not she replies) — after that, an
...
export async function handleSendMessage(request, env) {
  ...
  if (sender_type === "subscriber") {
    ...
    if (!activeSub) {
      // No active subscription — check whether the one free trial
      ...
      // First message to this model — consume the trial, allow it through.
